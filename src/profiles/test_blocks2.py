...
class Executor(BaseExecutor):
    ...
    def start(self):
        ...
        DIMS = [108*9, 1000, 1000, 1000, 1000, 1943]
        NUMS = [1, 1, 1, 1, 1, 1]
        FUNCS = [
            Rectifier, 
            Rectifier, 
        ...
        path_v = pjoin(PATH['fuel'], pfx+'_validate.hdf5')
        data = H5PYDataset(path, which_set='train', load_in_memory=True)
        data_v = H5PYDataset(path_v, which_set='validate', load_in_memory=True)
        ...
        main_loop = MainLoop(data_stream = data_stream, 
                algorithm=algo, 
                extensions=[monitor, monitor_v, FinishAfter(after_n_epochs=15), Printing()])
